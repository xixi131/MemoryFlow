# Phase 6 Windows Parity Contract

## Scope

This is a code-derived contract for the native macOS Phase 6 implementation.
`front-end/src/components/DynamicIslandWidget.tsx` and
`front-end/electron/main.cjs` are **Windows parity references only**, not
modification targets for this migration. The matrix records the presentation
that native mock scenarios must be able to reproduce; it does not claim a
