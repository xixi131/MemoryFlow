...
struct TopAttachmentMetrics: Equatable {
    ...
    func notchAlignedBodyWidth(for state: IslandVisualState) -> CGFloat? {
        ...
        case .activityCollapsed, .activityHoverCollapsed, .expandedMusic, .expandedApp, .loginRequired, .updatePrompt, .reminderBanner, .reminderBannerExpanded, .externalAgentNotification:
            return nil
        }
    }
}
