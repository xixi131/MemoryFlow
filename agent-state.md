...
## First pending task
* Record the Windows compact and activity shell token values in `docs/mac-island-visual-token-map.md`.
...
5. Read `docs/mac-island-visual-token-map.md`.
6. Read `front-end/src/components/DynamicIslandWidget.tsx` for the Windows path functions and visual constants.
7. Read `mac-island/MemoryFlowIsland/UI/` plus `mac-island/MemoryFlowIsland/Window/` only as needed for preview and top-band scaling integration.
8. Read `feature_list_summary.json` only if you need completed Phase 0 to Phase 2 history.
