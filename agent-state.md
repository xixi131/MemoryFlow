...
Phase 7 Settings/menu cleanup, Advanced Features lifecycle, login-required square, Sparkle core/policy/prompt, update download activity, and the unsigned GitHub CI/CD release foundation are complete. Remaining work is unsigned installation from CI artifacts and GitHub release acceptance. Existing real music-provider work in the current dirty worktree is not part of this queue.

## Queue status
`feature_list.json` contains 2 pending Phase 7 tasks. Start with `mac-phase7-unsigned-install-from-ci`, consume artifacts generated by `mac-island/release.sh`, and finish with `mac-phase7-github-release-acceptance`. Both earlier pending-task drafts are archived in `final_feature_list.json`.
