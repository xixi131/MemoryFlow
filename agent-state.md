...
The todo priority/input foundation, composer and editor simplification, web list cleanup, trend API, native data fidelity, read-only native detail, detail motion, and adaptive native synchronization are complete. The remaining queue continues with statistics routing/charting, web synchronization, and cross-surface acceptance. The previous `mac-phase7-github-release-acceptance` task remains archived unchanged. Existing real music-provider work and untracked Xcode user data are not part of this queue.

## Queue status
`feature_list.json` preserves 9 completed Phase 7 tasks, completes nine todo-improvement slices, and contains 4 pending todo-improvement tasks. Start with `todo-web-route-backed-segmented-shell`.
